...
self.onmessage = function(msg) {
  ...
  switch(msg.data.op) {
    case 0:
      generate(data, msg.data.realm, msg.data.seed, msg.data.pos);
      break;
  }
...
function generate(data, realm, seed, position) {
  if(!noise[""+seed]) {
    noise[""+seed] = new Noise(seed);
  }
  ...
  switch(realm) {
    case 'overworld-0.0.1':
      for(var x=0; x<64; x++) {
        for(var z=0; z<64; z++) {
          var height = (noise[""+seed].perlin2(position[0]/64 + x/64, position[2]/64 + z/64)+1)*32;
